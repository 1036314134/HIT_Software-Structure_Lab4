...
public class EventManager {
	...
	static TreeMap<Integer, List<Time>> temp = new TreeMap<Integer, List<Time>>();
	public int max = 1;
	/**
	 * @param day the day now
	...
	public int book(int day, int start, int end) {
		if(temp.get(day) == null) {
			List<Time> k = new ArrayList<>();
			k.add(new Time(start, end));
			temp.put(day, k);
			return max;
		}
		temp.get(day).add(new Time(start, end));
		int ans = 1;
		for (int i = 0; i <= 24; i++) {
			int	active = 0;
			for (Time t : temp.get(day)) {
				if (t.getStart() < i && i < t.getEnd()) {
					active++;
				}	
			}
			if(active > ans) {
